...
fun describe(result: TransferResult): String{
    when (result){
        is TransferResult.Success ->
            return "Transfer successful. Confirmation code: ${result.confirmationId}"
        is TransferResult.Failure ->
            return "Transfer failed because ${result.reason}."
    }
}
...
suspend fun fetchAccounts(): List<Account>{
    ...
    return listOf(Account(id = "101", name ="Bob", balance = 125.0), Account(id = "102", name = "Nick", balance = 869.5), Account(id = "103", name = "Jake", balance = -134.8))
}
...
fun main() = runBlocking {
    ...
    val negAccounts = accounts.filter{it.balance < 0}
    val totalMoneys = accounts.sumOf{it.balance}
    println("There are ${negAccounts.count()} negitive accounts: $negAccounts")
    println("The total for all accounts is ${totalMoneys.asCurrency()}.")
